...
def shops():
    ...
    items = Shop.query.order_by(desc(Shop.consume_cnt)).paginate(page, 10, False)

    add_form = ShopCreateForm()
    delete_form = MovieDeleteForm()
    temp_error_form = None
    temp_modify_form = ShopModifyForm()
        ...
        if current_user.is_authenticated and current_user.is_admin:
            ...
            if temp_modify_form.modify_submit.data and temp_modify_form.is_submitted():
                if not temp_modify_form.validate():
                    ...
                    return modify_shop(temp_modify_form)
            if delete_form.delete_submit.data and delete_form.validate_on_submit():
                to_delete = Shop.query.filter_by(id=int(delete_form.id.data)).first()
                db.session.delete(to_delete)
                flash("删除成功")
                return redirect(url_for("management.shops"))
        else:
    ...
    next_url = url_for('management.shops', page=items.next_num) if items.has_next else None
    prev_url = url_for('management.shops', page=items.prev_num) if items.has_prev else None
...
def add_shop(add_form):
    to_add = Shop(name=str(add_form.name.data).strip())
    db.session.add(to_add)
    flash("成功添加《"+str(add_form.name.data).strip()+"》")
    return redirect(url_for("management.shops"))


def modify_shop(temp_modify_form):
    target = Shop.query.filter_by(id=int(temp_modify_form.id.data)).first()

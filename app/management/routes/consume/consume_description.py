...
from app.management.forms.consume import ConsumeDescriptionCreateForm, ConsumeDescriptionModifyForm
...
from app.models.consume import ConsumeDescription


# 消费列表
@bp.route('/consume/descriptions', methods=['GET', 'POST'])
def consume_descriptions():
    page = request.args.get('page', 1, type=int)
    items = ConsumeDescription.query.order_by(desc(ConsumeDescription.consume_cnt)).paginate(page, 10, False)

    add_form = ConsumeDescriptionCreateForm()
    delete_form = MovieDeleteForm()
    temp_error_form = None
    temp_modify_form = ConsumeDescriptionModifyForm()
        ...
        if current_user.is_authenticated and current_user.is_admin:
            if add_form.create_submit.data and add_form.is_submitted():
                if not add_form.validate():
                    ...
                    return add_consume_description(add_form)
            if temp_modify_form.modify_submit.data and temp_modify_form.is_submitted():
                if not temp_modify_form.validate():
                    ...
                    return modify_consume_description(temp_modify_form)
            if delete_form.delete_submit.data and delete_form.validate_on_submit():
                to_delete = ConsumeDescription.query.filter_by(id=int(delete_form.id.data)).first()
                db.session.delete(to_delete)
                flash("删除成功")
                return redirect(url_for("management.consume_descriptions"))
        else:
    ...
    next_url = url_for('management.consume_descriptions', page=items.next_num) if items.has_next else None
    prev_url = url_for('management.consume_descriptions', page=items.prev_num) if items.has_prev else None
...
def add_consume_description(add_form):
    to_add = ConsumeDescription(name=str(add_form.name.data).strip())
    db.session.add(to_add)
    flash("成功添加《"+str(add_form.name.data).strip()+"》")
    return redirect(url_for("management.consume_descriptions"))


def modify_consume_description(temp_modify_form):
    target = ConsumeDescription.query.filter_by(id=int(temp_modify_form.id.data)).first()
...
def modify_form_constructor(items):
    ...
    for current_item in items.items:
        temp_form = ConsumeDescriptionModifyForm()
